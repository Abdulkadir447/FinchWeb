...
export function Navbar() {
  ...
  return <header className="sticky top-0 z-50 border-b border-ink-200/70 bg-[#f7f8f6]/90 backdrop-blur-md">
    ...
    {open && <div className="border-t border-ink-200 bg-[#f7f8f6] px-4 py-5 md:hidden"><nav className="container flex flex-col gap-4 text-sm"><a href="#product" onClick={() => setOpen(false)}>Product</a><a href="#workflow" onClick={() => setOpen(false)}>Workflow</a><a href="#access" onClick={() => setOpen(false)}>Access</a><Button onClick={() => document.querySelector('#access')?.scrollIntoView({ behavior: 'smooth' })}>Request access <ArrowUpRight size={15} /></Button></nav></div>}
  </header>
}
